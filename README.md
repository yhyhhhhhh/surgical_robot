...
- Throughout the repository, the name `my_ur3_project` only serves as an example and we provide a script to rename all the references to it automatically:

```bash
# Enter the repository
cd IsaacLabExtensionTemplate
# Rename all occurrences of my_ur3_project (in files/directories) to your_fancy_extension_name
...
```bash
python -m pip install -e exts/my_ur3_project
```
...
```json
{
    "python.analysis.extraPaths": [
        "<path-to-ext-repo>/exts/my_ur3_project"
    ]
}
```
