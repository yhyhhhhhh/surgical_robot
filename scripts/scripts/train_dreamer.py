...
def main(config):
	...
	if config.model_only:
		...
		while agent._step < config.steps + config.eval_every:
			train_round += 1
			target_step = config.steps + config.eval_every
			trained_step = int(agent._step)
			remain_step = max(0, int(target_step - trained_step))
			progress = 100.0 * float(trained_step) / float(target_step) if target_step > 0 else 100.0
			print(f"\n[TrainRound {train_round}] target={target_step} env-steps | chunk={config.eval_every}")
			print(f"[TrainProgress] 轮前: 已训练 {trained_step} | 剩余 {remain_step} | 进度 {progress:.2f}%")
			logger.scalar("训练进度/已训练环境步", trained_step)
			logger.scalar("训练进度/目标环境步", float(target_step))
			logger.scalar("训练进度/剩余环境步", remain_step)
			logger.scalar("训练进度/完成百分比", progress)
			...
			# 保留原行为：每轮采样固定 500 个环境步

			start_step_this_round = int(agent._step)
			...
			trained_step = int(agent._step)
			collected_this_round = trained_step - start_step_this_round
			remain_step = max(0, int(target_step - trained_step))
			progress = 100.0 * float(trained_step) / float(target_step) if target_step > 0 else 100.0
			print(
				f"[TrainProgress] 轮后: 已训练 {trained_step} | 本轮新增 {collected_this_round} | "
				f"剩余 {remain_step} | 进度 {progress:.2f}%"
			)
			logger.scalar("训练进度/已训练环境步", trained_step)
			logger.scalar("训练进度/目标环境步", float(target_step))
			logger.scalar("训练进度/剩余环境步", remain_step)
