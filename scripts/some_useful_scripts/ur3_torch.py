...
actions = torch.zeros(6).unsqueeze(0)  # 添加batch维度 → shape (1,7)
import math
import torch
# 预计算关节参数（元组存储加速访问）
JOINT_PARAMS = {
    ...
    'wrist_2_joint': (
        (-3.46, -0.92), (-math.pi, math.pi), 1.0
    ),
    'wrist_3_joint': (
        (-5.77, -0.46), (math.pi, -math.pi), 1.0
    )
}
...
def callback(msg):
    ...
    actions = torch.tensor(ur_angles, device='cuda').unsqueeze(0)
