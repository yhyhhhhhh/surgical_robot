...
third_last_joint_angle = 0.0  # 倒数第三个关节角 (用于 X 轴旋转)
...
def joint_callback(data):
    ...
    - 最后一个关节角：绕 Z 轴
    """
    global third_last_joint_angle,second_last_joint_angle, last_joint_angle
    if len(data.position) >= 2:
        third_last_joint_angle = data.position[-3]
...
def update(frame):
    ...
    global current_quat, third_last_joint_angle, second_last_joint_angle, last_joint_angle

    # 1) 将 /phantom/phantom/pose 的四元数转换为 Rotation
    R_pose = R.from_quat(current_quat)  # (x, y, z, w)

    # 2) 构造绕 X 轴旋转 (倒数第三个关节角)
    R_x = R.from_euler('x', third_last_joint_angle, degrees=False)
    
    # 3) 构造绕 Y 轴旋转 (倒数第二个关节角)
    R_y = R.from_euler('y', -second_last_joint_angle, degrees=False)

    # 4) 构造绕 Z 轴旋转 (最后一个关节角)
    R_z = R.from_euler('z', -last_joint_angle, degrees=False)

    # 5) 合成旋转: 先做 R_pose，再做 R_y，然后 R_z
    #    也可根据需求更改乘法顺序
