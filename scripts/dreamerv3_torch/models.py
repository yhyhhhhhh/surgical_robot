...
class WorldModel(nn.Module):
    ...
    @torch.no_grad()
    def encode_latent(self, obs):
        """
        将观测序列转换为 Dreamer latent state.
        输入:
            obs: dict
                obs["obs_key"]: (B,T,...)
    ...
    @torch.no_grad()
    def latent_step(self, state, action):
        """
        单步 latent dynamics rollout
        Args:
            state: dict
        ...
        Returns:
            next_state: dict
            feat: latent feature (B, feat_dim)
        """
        # RSSM prior step
        next_state = self.dynamics.img_step(state, action)
        # latent feature
        feat = self.dynamics.get_feat(next_state)
        return next_state, feat
    

    @torch.no_grad()
    def rollout_latent(self, init_state, actions):
        """
        在 world model 中 rollout latent trajectory
        Args:
            init_state: dict
                RSSM state
            actions: tensor (B, T, act_dim)
        Returns:
            states: list of RSSM states
            feats: tensor (B, T, feat_dim)
        """
        state = init_state
        feats = []
        states = []
