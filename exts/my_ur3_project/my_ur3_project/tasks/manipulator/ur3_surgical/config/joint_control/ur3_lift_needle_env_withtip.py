...
from .ur3_lift_needle_env_cfg import Ur3LiftNeedleEnvCfg
...
class Ur3LiftNeedleEnv(DirectRLEnv):
    ...
    def _resample_command(self, env_ids: Sequence[int]):
        ...
        self.pose_command_w[env_ids, :3], self.pose_command_w[env_ids, 3:] = combine_frame_transforms(
            ...
            self._right_robot.data.root_state_w[env_ids, 3:7],
            local_point[env_ids, :3]
        )
        print(local_point,self.pose_command_w)
    ...
    def _pre_physics_step(self, actions: torch.Tensor):
        ...
        if self.actions[:,6]==1:
            self.actions[:,6] = 0.2*self.actions[:,6].unsqueeze(0)
        else:
            self.actions[:,6] = 0.26*self.actions[:,6].unsqueeze(0)
    ...
    def _reset_idx(self, env_ids: torch.Tensor | None):
        ...
        self._right_robot.write_joint_state_to_sim(right_joint_pos, right_joint_vel, env_ids=env_ids)
        # 重新生成needle的位置
