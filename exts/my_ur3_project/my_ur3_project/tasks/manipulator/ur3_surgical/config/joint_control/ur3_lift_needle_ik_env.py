...
class Ur3LiftNeedleEnv(DirectRLEnv):
    ...
    def _pre_physics_step(self, actions: torch.Tensor):
        ...
        self.gripper_action = torch.tensor([0.46],device=self.device).unsqueeze(0)
        # self._robot_ik.process_actions(self.actions[:,0:6]) 
        # if self.actions[:,6]==1:
        #     self.gripper_action = 0.2*self.actions[:,6].unsqueeze(0)
        # else:
        #     self.gripper_action = 0.26*self.actions[:,6].unsqueeze(0)
