...
class Ur3LiftNeedleEnv(DirectRLEnv):
    ...
    def _pre_physics_step(self, actions: torch.Tensor):
        ...
        self.gripper_action = torch.where(
            ...
            torch.tensor([-0.08], device=self.device)
        )
        # self.gripper_action = torch.where(
    ...
    def _get_dones(self) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        if self.enable_draw:
            ...
            vis_obj = self._robot.data.body_pos_w[:, self.ee_id,:3].detach().cpu().numpy()
    ...
    def _get_rewards_11(self) -> torch.Tensor:
        ...
        # 取 object 的位置张量（假设 shape 为 (N, 3)，这里选择第一个）
        ee_pos_tensor =  self.ee_view.get_world_poses()[0] - self.scene.env_origins[:,:3]  # 例如 shape 为 (num_bodies, 3)
    ...
    def _get_observations1(self) -> dict:
        ...
        # 机械臂末端位置
        self.robot_local_pos =self._object.data.body_state_w[:,0,0:3] -  self.ee_view.get_world_poses()[0]
        ...
        # 相对于管口位置
        pos_ref =  self.ee_view.get_world_poses()[0] - self.pipe_top_pos
